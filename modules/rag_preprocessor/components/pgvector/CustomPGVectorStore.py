...
def _get_embedding_collection_store(collection_table: Optional[str] = None, 
                                    embeddings_table: Optional[str] = None,
                                    vector_dimension: Optional[int] = None) -> Any:
    from pgvector.sqlalchemy import Vector  # type: ignore

    # Generate unique class names based on table names
    collection_cls_name = f"CollectionStore_{collection_table or 'langchain_pg_collection'}"
    embedding_cls_name = f"EmbeddingStore_{embeddings_table or 'langchain_pg_embedding'}"

    # Dynamically create CollectionStore with the given table name
    # Define async methods outside the lambda to avoid SyntaxError
    def get_by_name(cls, session, name):
        return session.query(cls).filter(typing_cast(sqlalchemy.Column, cls.name) == name).first()

    async def aget_by_name(cls, session, name):
        result = await session.execute(select(cls).where(typing_cast(sqlalchemy.Column, cls.name) == name))
    ...
    async def aget_or_create(cls, session, name, cmetadata=None):
    ...
    DynamicCollectionStore = type(
        collection_cls_name,
        (Base,),
        {
            "__tablename__": collection_table or "langchain_pg_collection",
            "collection_uuid": sqlalchemy.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
    ...
    # Dynamically create EmbeddingStore with the given table name and FK to the above
    DynamicEmbeddingStore = type(
        embedding_cls_name,
        (Base,),
        {
            "__tablename__": embeddings_table or "langchain_pg_embedding",
            "id": sqlalchemy.Column(sqlalchemy.String, primary_key=True),
            "collection_uuid": sqlalchemy.Column(
                UUID(as_uuid=True),
                sqlalchemy.ForeignKey(f"{DynamicCollectionStore.__tablename__}.collection_uuid", ondelete="CASCADE"),
            ),
            "doc_id": sqlalchemy.Column(UUID(as_uuid=True), nullable=False),
            "collection": relationship(DynamicCollectionStore, back_populates="embeddings"),
            "embedding": sqlalchemy.Column(Vector(vector_dimension)),
            "document": sqlalchemy.Column(sqlalchemy.String, nullable=True),
            "cmetadata": sqlalchemy.Column(JSONB, nullable=True),
            "__table_args__": (
                sqlalchemy.Index(
                    f"ix_{embeddings_table or 'langchain_pg_embedding'}_cmetadata_gin",
                    "cmetadata",
    ...
    return (DynamicEmbeddingStore, DynamicCollectionStore)
...
class CustomPGVector(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        self.collection_table = collection_table # or "langchain_pg_collection"
        self.embeddings_table = embeddings_table # or "langchain_pg_embedding"
        ...
        with self._make_sync_session() as session:
            if ids is not None:
                ...
                if collection_only:
                    ...
                    stmt = stmt.where(
                        self.EmbeddingStore.collection_uuid == collection.collection_uuid
                    )
        ...
        **kwargs: Any,
        ...
        with self._make_sync_session() as session:  # type: ignore[arg-type]
            ...
            data = [
                {
                    ...
                    # Añadir el document_id procedente de los metadatos
            ...
            on_conflict_stmt = stmt.on_conflict_do_update(
                index_elements=["collection_uuid", "id"],
                set_={
    ...
    def _results_to_docs_and_scores(self, results: Any) -> List[Tuple[Document, float]]:
        ...
        # The result row contains the model instance as the first element, not as a key
        docs = [
            (
                Document(
                    id=str(result[0].id),
                    page_content=result[0].document,
    ...
    def _handle_field_filter(
        ...
        if isinstance(value, dict):
            ...
            # --- Aquí va la lógica especial para arrays JSONB ---
            if operator == "$contains" and field == "role_list":
                # Lógica especial para arrays JSONB
                # Si el valor no es una lista, lo convertimos
                filter_value = [filter_value] if not isinstance(filter_value, list) else filter_value
                return self.EmbeddingStore.cmetadata[field].contains(filter_value)
    ...
    def __query_collection(
        ...
        with self._make_sync_session() as session:  # type: ignore[arg-type]
            ...
            filter_by = [self.EmbeddingStore.collection_uuid == collection.collection_uuid]
            if filter:
                if self.use_jsonb:
            ...
            results: List[Any] = (
                ...
                .join(
                    ...
                    self.EmbeddingStore.collection_uuid == self.CollectionStore.collection_uuid,
                )
                .limit(k)
                .all()
            )
